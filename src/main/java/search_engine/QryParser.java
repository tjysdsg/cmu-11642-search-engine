...
public class QryParser {
     ...
     */
    public static Qry createOperator(String operatorName) {
        Qry operator = null;
        ...
        if (substrings.length > 1) operatorDistance = Integer.parseInt(substrings[1]);
        ...
        switch (operatorNameLowerCase) {
            ...
            case "#wand":
                ...
                break;
            // TODO: Add new query operators here
     ...
     */
    public static Qry getQuery(String queryString) throws IOException, IllegalArgumentException {
     ...
     * @throws IllegalArgumentException Query syntax error.
     */
    private static Qry parseString(String queryString) throws IOException, IllegalArgumentException {
        ...
        if ((countChars(queryString, '(') == 0) || (countChars(queryString, '(') != countChars(queryString, ')')) || (indexOfBalencingParen(queryString) != (queryString.length() - 1))) {
        ...
        while (queryString.length() > 0) {
            ...
            //  weight first.
            double weight = 0;
            if (queryTree instanceof IWeightedSop) {
                PopData<Double, String> p = popNumber(queryString);
                weight = p.getPopped();
            ...
            for (Qry qarg : qargs) {
                if (queryTree instanceof IWeightedSop) {
                    ((IWeightedSop) queryTree).appendWeightedArg(qarg, weight);
                } else {
                    queryTree.appendArg(qarg);
                }
            }
        }
    ...
    /**
     * Remove a number from an argument string.  Return the number and
     * the modified argument string.
     *
     * @param argString A partial query argument string, e.g., "0.6 a 0.4 b".
     * @return PopData<Double, String>
     * The term string and the modified argString (e.g., 0.6 and "a 0.4 b").
     */
    static private PopData<Double, String> popNumber(String argString) {
        String[] substrings = argString.split("[ \t\n\r]+", 2);
        double num = Double.parseDouble(substrings[0]);
        ...
        return new PopData<Double, String>(num, argString);
    }
